...
- ジャーナルを1件1行で保存し、旧版の複数行記録もDoctorで読めるようにしました。`seq`/`sequence`の不一致と、REHOME後に古い復旧ファイルが残ってFAILになる問題も修正しました。
- 見出し・本文・通知の背景色漏れを修正し、メイン中央へ選択中ワーカーの詳細カードを追加しました。WORKERSで複数台をタッチしてグループと専用ドック・ベイを作成できます。
- メイン画面のボタンを画面サイズにかかわらず2行高にし、ワーカー一覧をPREV/NEXT付きの専用ページへ分離しました。MOREも2ページに分けています。
- 高さに余裕のある画面では、ボタンの文字行の上へ同色の1行を追加し、上側もタッチできるようにしました。
