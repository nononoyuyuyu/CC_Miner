...
- 遠隔コンソールを既定OFFで追加。controllerId（controller pin）固定、採掘停止・ドック確認、セッション時間／出力上限／監査件数、読み取りコマンドallowlist、allowShellの別確認を設定と実行時に適用。CC rednetは暗号化されないため初心者向け手順へ危険性を追記
