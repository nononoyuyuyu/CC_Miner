...
marker(setup, "Field discard side", "discard direction explanation")
marker(setup, "Inventory on this side makes world drop skip.", "discard side safety explanation")
marker(setup, "REMOTE CONSOLE SAFETY", "remote console strong warning")
marker(setup, "Enable safe remote console", "safe remote console default prompt")
marker(setup, "Enable allowShell (unsafe)", "remote shell separate confirmation")
marker(setup, "ID 0 allows all controllers with this key.", "same-key controller explanation")
marker(setup, "allowShell requires one fixed controller ID.", "arbitrary shell controller pin guard")
marker(setup, "Controller ID (0=same key)", "safe remote controller id prompt")
marker(setup, "Controller computer ID (1-65535)", "remote controller id prompt")
assert(not setup:find("[\128-\255]"), "setup.lua must stay ASCII for the CC:Tweaked terminal font")
