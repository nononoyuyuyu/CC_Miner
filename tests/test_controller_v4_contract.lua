...
for _, item in ipairs({
  ...
  { 'noticeState.createGroupFromWorkers = function()', "touch group creation" },
  { 'Select at least two workers for a group.', "group minimum selection" },
  { 'Group creation needs GPS CAL on every selected worker.', "group GPS safety" },
  { 'registerDock(dockId, resource)', "automatic worker dock registration" },
  { 'registerBay(bayId, resource)', "automatic worker bay registration" },
  { 'registerGroup(groupId, ids, {', "selected worker group registration" },
  { '{ "GROUP", function() noticeState.groupSelectMode = true;', "worker group selection entry" },
  { '{ "CREATE", noticeState.createGroupFromWorkers', "worker group create button" },
  ...
  { 'common.setColors(colors and colors.black or nil, colors and colors.white or nil)', "header background reset" },
  { 'common.fit(prefix .. notice, width), noticeColor, colors and colors.black or nil', "notice black background" },
  { 'local cardBottom = firstRow - 2', "selected worker detail card" },
  { 'Inventory: %s used / %s empty / %s items', "selected worker inventory card" },
