...
marker(discardCandidate, "if not explicitAllowlisted(name, policy) then return false end", "exact custom allowlist check")
marker(discardCandidate, "if isTorchItemName(name) or isFuelItem(slot) or isOreOrRawMaterial(name) then return false end", "custom hard protected material check")
marker(discardCandidate, "if isSealItemName(name) then", "custom seal excess support")
...
marker(discardCandidate, "local keep = math.max(1, tonumber(config.discard and config.discard.retainSealTarget) or 64)", "stone seal reserve target")
marker(discardCandidate, "return totalSeal > keep", "seal reserve gate")
marker(worker, "local function homeRetainCounts()", "home material target planner")
marker(worker, "local keepCounts = homeRetainCounts()", "home target application")
marker(worker, "local removable = count - keepCount", "home excess-only unload")
marker(worker, "local dropped = discardDrop(removable)", "home partial excess discard")
marker(worker, "local dropped = outDrop(removable)", "home partial excess output")
...
marker(worker, "if command == \"discard_config\" then", "remote discard settings command")
marker(worker, "Discard settings can only be changed while the worker is at its dock.", "discard settings home gate")
